...
function App() {
  ...
  const handleDeletePerson = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/deletePerson/${personId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleDeleteTransaction = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/deleteTransaction/${transactionId}`,
        {
          method: "DELETE",
        }
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div>
            ...
            >
              ...
              {Array.from(new Set(transactions.map((t) => t.person_name))).map(
                (name) => (
                ...
                >
                  ...
                  <td
                    className={`py-2 px-4 font-medium ${
                      transaction.is_give ? "text-red-600" : "text-green-600"
                    }`}
                  >
                    {transaction.is_give ? "-" : "+"}₹{transaction.amount}
                  </td>
